MODULE Plume;
(*
 * plume - simple markdown to PDF/HTML/EPUB/DOCX/RTF converter. Every
 * format is generated natively (no pandoc/pdflatex/LibreOffice shellout).
 * Usage: plume [-o outfile] [--html|--pdf|--epub|--docx|--rtf] <input.md>
 *   --html   native HTML output
 *   --pdf    native PDF output (default)
 *   --epub   native EPUB 3 ebook
 *   --docx   native Word document (a real .docx, via Markdown.mod's
 *            Docx* renderer — see DoDocx below)
 *   --rtf    RTF document (opens in Word/LibreOffice)
 *)
...
PROCEDURE DoDocx;
(* Real DOCX — a proper .docx a word processor can open, not the --rtf
   alias this used to mean. Five small ZIP members (four fixed, one
   generated by streaming the input through Markdown.mod's Docx*
   renderer) — same shape as DoEpub above, and the same pipeline
   ostar.mod's ^K I / ^P D drive; see Markdown.mod's DOCX section. *)
VAR f: Files.File;
    tmpTypes, tmpRels, tmpDocRels, tmpStyles, tmpDoc: ARRAY LLEN OF CHAR;
    ok: BOOLEAN;
BEGIN
  MakeTempPath(outFile, ".ctypes.tmp", tmpTypes);
  MakeTempPath(outFile, ".rels.tmp", tmpRels);
  MakeTempPath(outFile, ".docrels.tmp", tmpDocRels);
  MakeTempPath(outFile, ".styles.tmp", tmpStyles);
  MakeTempPath(outFile, ".doc.tmp", tmpDoc);
  ...
  IF ~ok THEN Out.String("plume: DOCX packaging failed"); Out.Ln; HALT(1) END
END DoDocx;
...
PROCEDURE Usage;
BEGIN
  Out.String("Usage: plume [-o outfile] [--html|--pdf|--epub|--docx|--rtf] <input.md>"); Out.Ln;
  ...
  Out.String("  --docx   Word document (native, a real .docx)"); Out.Ln;
...
BEGIN
  ...
  WHILE i <= Args.Count() DO
    ...
    ELSIF Strings.Compare(arg, "--rtf")  = 0 THEN fmt := FMTRTF
  ...
  IF outFile[0] = 0X THEN
    ...
    ELSIF fmt = FMTRTF  THEN SetExt(inFile, ".rtf",  outFile)
    ELSIF fmt = FMTEPUB THEN SetExt(inFile, ".epub", outFile)
    ELSIF fmt = FMTDOCX THEN SetExt(inFile, ".docx", outFile)
  ...
  IF fmt = FMTDOCX THEN
    (* Its own multi-file/ZIP pipeline — see DoDocx above. *)
    DoDocx;
    Files.Close(inF);
    Out.String("plume: wrote "); Out.String(outFile); Out.Ln;
    HALT(0)
  ...
  ELSIF fmt = FMTRTF THEN Markdown.RtfHeader
  ...
  WHILE ~inR.eof DO
    ...
    ELSIF fmt = FMTRTF THEN Markdown.RtfLine(line)
  ...
  ELSIF fmt = FMTRTF THEN Markdown.RtfFooter
