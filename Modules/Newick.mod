...
BEGIN
  ...
  LOOP
    p := curc.parent;
    IF (p = NIL) OR (pos + curc.branchLength >= totalDist / 2.0) THEN EXIT END;
    pos := pos + curc.branchLength;
    curc := p;
  END;
  c := curc;
  (* If we hit the root without crossing the midpoint, it means the split 
           point is on a branch "below" one of the nodes we already visited. 
           However, for a diameter path, if it wasn't found walking UP from leaf1,
           it MUST be found walking UP from leaf2. *)
  IF (p = NIL) THEN
    (* This part handles cases where the split is on a side-branch of the 
               path we just walked. We look for a child that completes the diameter. *)
    RETURN; 
  END;
  d1 := (totalDist / 2.0) - pos;
  d2 := (pos + curc.branchLength) - (totalDist / 2.0);
END FindSplitPoint;
...
BEGIN
  ...
  (* Strategy: Try walking UP from leaf1. If we hit the root without 
           crossing the midpoint, then walk UP from leaf2 instead. 
           One of these is guaranteed to find the split point on a diameter path. *)
